...
class DoorbellResp(implicit val p: Parameters)
  ...
  val instructions =UInt(width=32)
...
class KomodoMatchUnit(id: Int)(implicit val p: Parameters) extends Module
  ...
  val triggered = isMatch(comp.valid & io.commitLog.valid, comp.bits, io.commitLog.bits) & (io.commitLog.bits.priv === UInt(0) | io.commitLog.bits.priv === UInt(1) | io.commitLog.bits.priv === UInt(2) | io.commitLog.bits.priv === UInt(3)) & io.walk_mu
  comp.bits.maskCare.priv        := UInt(0)
  comp.bits.maskDontCare.priv    := UInt(3)
  ...
  when (enable) {
    printf("[KOMODO] Enable MU%d mask.care.inst: 0x%x   mask count %d\n", UInt(id), comp.bits.maskCare.inst,comp.bits.countMatch)
  ...
  when (fireCountMatch) {
    ...
    io.resp.doorbell.bits.instructions := io.commitLog.bits.inst
